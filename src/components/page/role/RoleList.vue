...
<script>
    import {selectRoleList, insertRole, updateRole, updateRoleStatus} from '../../../api/role';
    ...
    export default {
        ...
        methods: {
            // 获取 easy-mock 的模拟数据
            getData() {
                selectRoleList(this.query).then(res => {
            ...
            handleDelete(index, row) {
                ...
                this.$confirm('确定要删除[' + row.name + ']吗？', '提示', {
                    ...
                    .then(() => {
                        updateRoleStatus({ids: row.id, status: 2}).then(res => {
                            if (res.code == '200') {
            ...
            delAllSelection() {
                ...
                this.$confirm('确定要删除[' + name + ']吗？', '提示', {
                    ...
                    .then(() => {
                        updateRoleStatus({ids: str, status: 2}).then(res => {
                            if (res.code == '200') {
                                this.$message.success(res.msg)
                                this.addVisible = false;
                                this.getData();
